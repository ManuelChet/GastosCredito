...
function validarCamposCondicionales() {
  ...
  if (tipo === 'derechos_posesorios') {
    ...
    // Mostrar ubicacion para montos mayores a Q400,000
    if (monto > 400000) {
...
btnCalcular.addEventListener('click', function () {
  ...
  if (tipo === 'microcredito' || tipo === 'credito_seguro_columna') {
   //CAMBIAR PARTE: MICROCRÉDITO 1% DE GASTOS ADMIN, Y PAGO DE SEGURO
   //CAMBIAR PARTE: SEGURO COLUMNA 1.5% DE GASTOS ADMIN, 1% AHORRO DISPONIBLE Y PAGO DE SEGURO.
...
if (tipo === 'derechos_posesorios') {
  ...
  if (monto >= 1000 && monto <= 50000) {
    ...
    if (ubicacionDP.value === 'solola') {
      // Q1,000 por cada escritura en Solola
      avaluo = 1000 * numEscrituras;
      infoAdicional.push(`Ubicacion avaluo: Solola (Q 1,000.00 x ${numEscrituras} escritura${numEscrituras > 1 ? 's' : ''} = Q ${formatoQuetzales(avaluo)})`);
    } else {
      // Q1,500 por cada escritura fuera de Solola
      avaluo = 1500 * numEscrituras;
      infoAdicional.push(`Ubicacion avaluo: Fuera de Solola (Q 1,500.00 x ${numEscrituras} escritura${numEscrituras > 1 ? 's' : ''} = Q ${formatoQuetzales(avaluo)})`);
    }
  }
